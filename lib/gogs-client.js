 ...
 * @constructor
 */
function API (apiUrl, userAgent, requester) {
  var _this = this;
  _this.apiUrl = apiUrl;

  // Dependency injection. Allow a custom request module.
  var request = requester ? requester(apiUrl) : Requester(apiUrl, userAgent);
  ...
  _this.createToken = function (token, user) {
    ...
    return request('users/' + user.username + '/tokens', user, params).then(stat.checkCreatedResponse);
  };
